#!/usr/bin/env python3
"""TP/SL grid on integrated causal Exness Zero Spread strategy.

Frozen:
- Strict trigger
- 3/5 tick strong momentum
- Reverse
- causal adaptive vol Q25-Q75 using prior qualifying Main signals
- 13-15 UTC
Only TP/SL varies.
"""
...
OUT=ROOT/"results"/"exness-tpsl-grid-oos"
...
HOLD=120000; HOURS={13,14,15}; COMMS=(0.0,.03)
TPS=(.60,.70,.80,.90); SLS=(.50,.60,.70)
...
def replay(ticks,tp,sl):
    ...
    for t,a,bd in ticks:
        ...
        if pos is not None:
            hold=t-pos["entry_t"]
            if pos["dir"]==1:
                px=bd; pnl=px-pos["entry"]; done="SL" if px<=pos["sl"] else ("TP" if px>=pos["tp"] else None)
            else:
                px=a; pnl=pos["entry"]-px; done="SL" if px>=pos["sl"] else ("TP" if px<=pos["tp"] else None)
            if done is None and hold>=HOLD:done="TIME"
            if done:
                pos.update(pnl=pnl,reason=done,hold_ms=hold); out.append(pos); pos=None
            continue
...
for tp in TPS:
    for sl in SLS:
        key=f"TP{tp:.2f}_SL{sl:.2f}"
        item={}
        pfs=[]
        for pname,ticks in loaded.items():
            tr=replay(ticks,tp,sl)
            mm={f"{c:.2f}":met(tr,c) for c in COMMS}
            item[pname]=mm
            pfs.append(mm["0.00"]["PF"])
        item["score"]={"mean_PF":sum(pfs)/len(pfs),"min_PF":min(pfs)}
        res["grid"][key]=item
        rows.append([key,item["score"]["mean_PF"],item["score"]["min_PF"],
                     item["OOS1"]["0.00"]["Npd"],item["OOS1"]["0.00"]["WR"],item["OOS1"]["0.00"]["PF"],item["OOS1"]["0.03"]["PF"],
                     item["OOS2"]["0.00"]["Npd"],item["OOS2"]["0.00"]["WR"],item["OOS2"]["0.00"]["PF"],item["OOS2"]["0.03"]["PF"]])
(OUT/"summary.json").write_text(json.dumps(res,indent=2))
with (OUT/"grid.csv").open("w",newline="") as f:
    w=csv.writer(f);w.writerow(["variant","meanPF","minPF","OOS1_Npd","OOS1_WR","OOS1_PF","OOS1_PF_c03","OOS2_Npd","OOS2_WR","OOS2_PF","OOS2_PF_c03"]);w.writerows(rows)
