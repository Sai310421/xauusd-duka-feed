#!/usr/bin/env python3
"""State-dependent exit A/B on frozen Exness Zero Spread entry stack.

Frozen entry:
- Strict trigger
- m3/m5 strong momentum
- Reverse
- causal adaptive vol Q25-Q75
- 13-15 UTC
Only exits vary.
"""
...
OUT=ROOT/"results"/"exness-state-exit-oos"
...
def replay(ticks,mode):
    b=B(); seed=None; pos=None; out=[]; last=-10**18
    ...
    for t,a,bd in ticks:
        ...
        if len(mids)>=20:
            xs=list(mids)[-20:]
            ret=sum(abs(xs[i]-xs[i-1]) for i in range(1,len(xs)))/19

        if pos is not None:
            hold=t-pos["entry_t"]
            if pos["dir"]==1:
                px=bd; pnl=px-pos["entry"]
                done="SL" if px<=pos["sl"] else ("TP" if px>=pos["tp"] else None)
            else:
                px=a; pnl=pos["entry"]-px
                done="SL" if px>=pos["sl"] else ("TP" if px<=pos["tp"] else None)
            ...
            if done is None:
                # E1: MFE trail after +0.20, give back 0.12
                if mode in ("MFE_TRAIL","HYBRID") and pos["mfe"]>=.20 and pnl<=pos["mfe"]-.12:
                    done="MFE_TRAIL"

                # E2: momentum stall/turn against reverse position.
                # For long, negative short-term bid momentum is adverse; vice versa for short.
                if done is None and mode in ("MOM_STALL","HYBRID") and hold>=500:
                    m3=b.mom(3)
                    adverse=(m3*pos["dir"]<0 and abs(m3)>=.04)
                    if adverse: done="MOM_STALL"

                # E3: 3 consecutive adverse bid ticks.
                if done is None and mode in ("OPP3","HYBRID") and hold>=500:
                    c=b.cons()
                    adverse3=(abs(c)>=3 and (1 if c>0 else -1)==-pos["dir"])
                    if adverse3: done="OPP3"

                # E4: First-passage early loss cut if trade never achieves +0.05
                # and reaches -0.25 after 3s.
                if done is None and mode in ("FP_EARLY","HYBRID") and hold>=3000:
                    if pos["mfe"]<.05 and pnl<=-.25:
                        done="FP_EARLY"

                if done is None and hold>=HOLD: done="TIME"

            if done:
                pos.update(pnl=pnl,reason=done,hold_ms=hold)
        ...
        if age>10000:
            seed=None; continue
        if a-bd>SPREAD_CAP or not(FOLLOW_MIN_MS<=age<=FOLLOW_MAX_MS):
            continue
        ...
        if len(hist)>=50:
            sv=sorted(hist)
            q25=sv[int((len(sv)-1)*.25)]
            q75=sv[int((len(sv)-1)*.75)]
            pass_vol=(q25<ret<=q75)
        hist.append(ret)

        hr=dt.datetime.fromtimestamp(t/1000,dt.timezone.utc).hour
        if not(pass_vol and hr in HOURS):
            seed=None; continue

        td=-d; e=a if td==1 else bd
        pos={"entry_t":t,"dir":td,"entry":e,
             "sl":e-SL if td==1 else e+SL,
             "tp":e+TP if td==1 else e-TP,
             "mfe":0.0,"mae":0.0}
        last=t; seed=None
...
def met(rows,c):
    ...
    return {"N":len(rows),"Npd":len(rows)/5,"WR":len(w)/len(rows) if rows else 0,
            "PF":pf,"EV":sum(pn)/len(pn) if pn else 0,
            "Net":sum(pn),"MaxDD":mdd,"max_loss_streak":mx}

MODES=("BASE","MFE_TRAIL","MOM_STALL","OPP3","FP_EARLY","HYBRID")
periods={
...
for mode in MODES:
    item={}
    pfs=[]
    for pname,ticks in loaded.items():
        tr=replay(ticks,mode)
        mm={f"{c:.2f}":met(tr,c) for c in COMMS}
        item[pname]=mm
        pfs.append(mm["0.00"]["PF"])
    item["score"]={"mean_PF":sum(pfs)/2,"min_PF":min(pfs)}
    res["modes"][mode]=item
    rows.append([mode,item["score"]["mean_PF"],item["score"]["min_PF"],
                 item["OOS1"]["0.00"]["Npd"],item["OOS1"]["0.00"]["WR"],item["OOS1"]["0.00"]["PF"],item["OOS1"]["0.03"]["PF"],item["OOS1"]["0.00"]["MaxDD"],
                 item["OOS2"]["0.00"]["Npd"],item["OOS2"]["0.00"]["WR"],item["OOS2"]["0.00"]["PF"],item["OOS2"]["0.03"]["PF"],item["OOS2"]["0.00"]["MaxDD"]])

(OUT/"summary.json").write_text(json.dumps(res,indent=2))
with (OUT/"exit_grid.csv").open("w",newline="") as f:
    w=csv.writer(f)
    w.writerow(["mode","meanPF","minPF","OOS1_Npd","OOS1_WR","OOS1_PF","OOS1_PF_c03","OOS1_MaxDD","OOS2_Npd","OOS2_WR","OOS2_PF","OOS2_PF_c03","OOS2_MaxDD"])
    w.writerows(rows)
