#!/usr/bin/env python3
"""Exness OOS time gate overlay on frozen Main+Vol candidate."""
...
OUT=ROOT/"results"/"exness-timegate-oos"
...
vals=sorted(r["ret_abs20"] for r in main)
q=lambda p: vals[min(len(vals)-1,int((len(vals)-1)*p))]
q25,q50,q75=q(.25),q(.5),q(.75)
vol=[r for r in main if q25<r["ret_abs20"]<=q75]

# Predefined adjacent UTC windows.
windows={
 "H13_15":set((13,14,15)),
 "H14_16":set((14,15,16)),
 "H15_17":set((15,16,17)),
 "H16_17":set((16,17)),
 "H13_17":set((13,14,15,16,17)),
 "H07_12":set(range(7,13)),
}
res={"period":"2026-09-14..18 OOS","vol_q2_q3":[q25,q75],"base_N":len(main),"vol_N":len(vol),"windows":{}}
for name,hs in windows.items():
    rr=[r for r in vol if r["hour"] in hs]
    res["windows"][name]={f"{c:.2f}":met(rr,c) for c in COMMS}
